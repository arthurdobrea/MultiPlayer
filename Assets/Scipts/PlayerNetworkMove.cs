...
public class PlayerNetworkMove : NetworkBehaviour
{
    ...
    private float disbleJumpTime = 0.2f;
    private float disableJumpCounter = 0;
    ...
    void ProcessInputs()
    {
        ...
        if (moveY >= 0.5 && isGrounded && disableJumpCounter == 0)
        {
            isJumping = true;
        ...
        {
            disableJumpCounter += Time.deltaTime;
            if (disableJumpCounter >= disbleJumpTime)
            {
                disableJumpCounter = 0;
            }
        }
